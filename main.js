...
function loadImageFile(file) {
  ...
  displayCapturedImageUrl(url);
}

function displayCapturedImageUrl(url) {
  const img = new Image();
  var timer = setTimeout(function() {
    // Fallback: just draw what we know
    console.warn('Image onload timeout, using fallback');
    drawToCanvas(img, img.naturalWidth || 300, img.naturalHeight || 300);
  }, 3000);
  
  img.onload = function() {
    clearTimeout(timer);
    drawToCanvas(this, this.width, this.height);
    URL.revokeObjectURL(url);
  };
  img.onerror = function() {
    clearTimeout(timer);
    console.warn('Image load error, using fallback size');
    drawToCanvas(img, 300, 300);
  };
  img.src = url;
}
...
function drawToCanvas(img, w, h) {
  ...
  try { ctx.drawImage(img, 0, 0, w, h); } catch(e) {
    // If draw fails (not fully loaded), just show the canvas area
    ctx.fillStyle = '#333';
  ...
  setTimeout(doFaceAnalysis, 500);
}



// ═══════ FACE ANALYSIS ═══════
document.getElementById('btnAnalyze').addEventListener('click', doFaceAnalysis);

async function doFaceAnalysis() {
  await loadModels();
  if (!modelsLoaded) return;
  showLoading('分析面部特征...');
  try {
...
function showCharDetail(name) {
  ...
  container.innerHTML = '' +
    '<div style="text-align:center;padding:40px 20px">' +
      ...
      '<div style="font-size:13px;color:var(--sub);margin-bottom:20px">正在搜索热门Cos图片...</div>' +
      '<div class="spinner" style="margin:0 auto"></div>' +
    '</div>';
  document.getElementById('resultView').classList.remove('active');
  document.getElementById('detailView').classList.add('active');
  
  fetch((typeof apiBase !== 'undefined' ? apiBase : '') + '/search', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({character: name, count: 6}),
    signal: AbortSignal.timeout(15000)
    ...
    if (images.length === 0) {
      container.innerHTML = '' +
        '<div style="text-align:center;padding:40px 20px">' +
          ...
          '<div style="font-size:13px;color:var(--sub)">暂未收录Cos图片</div>' +
        '</div>';
      return;
    }
    var gridHtml = '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:16px;max-width:400px;margin:0 auto">';
    var maxImg = Math.min(images.length, 6);
    for (var i = 0; i < maxImg; i++) {
      var imgUrl = (typeof apiBase !== 'undefined' ? apiBase : '') + images[i].url;
      gridHtml += '<div style="border-radius:12px;overflow:hidden;aspect-ratio:3/4;border:1px solid rgba(255,255,255,.06)">' +
        '<img src="' + imgUrl + '" style="width:100%;height:100%;object-fit:cover" loading="lazy" onerror="this.parentElement.style.display=\'none\'"></div>';
    }
    gridHtml += '</div>';
    container.innerHTML = '' +
      ...
      '<div style="text-align:center;padding:10px 0 16px">' +
        ...
        '<div style="font-size:12px;color:var(--sub)">找到 ' + images.length + ' 张Cos图</div>' +
      '</div>' +
      gridHtml;
  }).catch(function(e) {
    container.innerHTML = '' +
      '<div class="back-btn" onclick="closeDetail()">← 返回</div>' +
      '<div style="text-align:center;padding:40px 20px;color:var(--sub);font-size:13px">加载失败</div>';
  });
}
...
// ═══════ 热门角色 ═══════
(async function loadHotChars() {
  try {
    var res = await fetch('hot-chars.json?_t=' + Date.now());
    var data = await res.json();
    var chars = data.popular || [];
    var container = document.getElementById('hotCharsRow');
    if (!container) return;
    container.innerHTML = chars.map(function(c) {
      return '<div style="flex:0 0 130px;scroll-snap-align:start;border-radius:14px;overflow:hidden;background:var(--card);border:1px solid rgba(255,255,255,0.06);cursor:pointer" onclick="showCharDetail(\'' + c.name + '\')">' +
        '<div style="width:130px;height:170px;overflow:hidden;position:relative;background:' + (c.color || '#333') + '">' +
          '<img src="' + c.img + '" style="width:100%;height:100%;object-fit:cover" loading="lazy" onerror="this.style.display=\'none\';this.parentElement.innerHTML=\'<div style=\\"display:flex;align-items:center;justify-content:center;height:100%;font-size:48px;opacity:0.3\\">' + c.name[0] + '</div>\'">' +
        '</div>' +
        '<div style="padding:8px 10px">' +
          '<div style="font-size:14px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + c.name + '</div>' +
          '<div style="font-size:11px;color:var(--sub);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + (c.likes || c.series) + '</div>' +
        '</div>' +
      '</div>';
    }).join('');
  } catch(e) {
    console.log('Hot chars load failed:', e);
  }
})();
