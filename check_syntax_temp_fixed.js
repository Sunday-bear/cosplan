...
// Wait for page to fully render, then load models in background
window.requestIdleCallback ? 
  window.requestIdleCallback(function() { loadModels(); }, {timeout: 5000}) :
  setTimeout(function() { loadModels(); }, 3000);
...
function displayCapturedImage(img) {
  // Auto-trigger face analysis once models are ready
  var checkCount = 0;
  function waitAndAnalyze() {
    if (modelsLoaded && typeof loadModels !== 'undefined') {
      document.getElementById('btnAnalyze').click();
    } else if (checkCount < 200) {
      checkCount++;
      setTimeout(waitAndAnalyze, 500);
    }
  }
  waitAndAnalyze();
